    ...
    public class ColorManager
    {
        ...
        public List<Color> ListColors(string type, string colorCode, decimal requestAmount, string unitOfMeasurement)
        {
            ...
            if (Common.ConfigVariable("DisplaySalesLinks") != null && Common.ConfigVariable("DisplaySalesLinks") == "true")
            {
                try
                {
                    colorURL = type == "Water Base" ? (Common.ConfigVariable("CMSSalesPortalURL")) : (type == "PC" ? Common.ConfigVariable("PCSalesPortalURL") : "");
                }
                catch
                {
                }
            ...
            else
            {
                foreach (XElement color in baseColorList)
                {
                    try
                    {
                        string thisColorType = color.Descendants("type").FirstOrDefault().Value;
        ...
        public List<String> ListColorTypes()
        {
            ...
            {
                if ((colorTypes == null || !colorTypes.Contains(colorType.Value)) && allowedColorTypeList != null && allowedColorTypeList.Contains(colorType.Value))
                    colorTypes.Add(colorType.Value);
            }

            if (!colorTypes.Contains("Discharge Base"))
                colorTypes.Add("Discharge Base");

            if (!colorTypes.Contains("Discharge Boost (+5%)"))
                colorTypes.Add("Discharge Boost (+5%)");
